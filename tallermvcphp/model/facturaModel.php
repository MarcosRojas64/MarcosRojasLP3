<?php
 ...
 $sql = 'SELECT f.numero,DATE_FORMAT(f.fecha, "%d/%m/%Y") fecha,concat(e.nombre,"
",e.apellido) cliente,e.cin,SUM(d.cantidad*d.importe) total FROM facturas f JOIN
detallefacturas d ON f.numero=d.numero JOIN estudiantes e ON f.idEstudiante=e.idEstudiante
WHERE f.anulada=0 GROUP BY f.numero ORDER BY f.numero DESC';
...
(0,:fecha,:idEstudiante,:idFormaPago,0,:idUsuario)';
 $statement = $this->PDO->prepare($sql);
 ...
 $sql = 'INSERT INTO detallefacturas VALUES
(:numero,:idCurso,:cantidad,:importe)';
 $statement = $this->PDO->prepare($sql);
...
FROM estudiantes e ORDER BY e.nombre';
 $statement = $this->PDO->prepare($sql);
 return ($statement->execute()) ? $statement->fetchAll() : false;
 }

 public function cargarEstudiantesID($idEstudiante) {
 $sql = 'SELECT e.idEstudiante,concat(e.nombre," ",e.apellido) razonsocial,e.cin,
c.nombre ciudad FROM estudiantes e JOIN ciudades c ON e.idCiudad = c.idCiudad WHERE
e.idEstudiante = :idEstudiante ORDER BY e.nombre';
 $statement = $this->PDO->prepare($sql);
 $statement->bindParam(':idEstudiante',$idEstudiante);
 return ($statement->execute()) ? $statement->fetch() : false;
 }
 ...
 $sql = 'SELECT f.idFormaPago,f.descripcion FROM formapagos f ORDER by
f.idFormaPago';
 $statement = $this->PDO->prepare($sql);
 return ($statement->execute()) ? $statement->fetchAll() : false;
 }
