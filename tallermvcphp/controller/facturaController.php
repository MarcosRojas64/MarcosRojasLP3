<?php
 ...
 return ($this->model->buscar($factura) != false) ? $this->model->buscar($factura)
: false;
 }
 ...
 return ($this->model->listarCursos()!= false) ? $this->model->listarCursos() :
false;
 }
...
$id = $this->model->insertarDetalle($numero,$idCurso,$cantidad,$importe);
 ...
 public function update($numero){
    return ($this->model->actualizar($numero) != false) ? header('location:
./index.php') : false;
}
 }
 ?>