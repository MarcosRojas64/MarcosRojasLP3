...
!$modelProd=="" && !$marcaProd=="" && !$stockProd=="" && !$codePProd=="" && !$_FILES['img']
['name']==""){
 $verificar= ejecutarSQL::consultar("select * from producto where
CodigoProd='".$codeProd."'");
 $verificaltotal = mysqli_num_rows($verificar);
 if($verificaltotal<=0){
 if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/imgproducts/".$_FILES['img']['name'])){
 if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat,
Precio, Modelo, Marca, Stock, RUCProveedor, Imagen, Nombre",
"'$codeProd','$nameProd','$cateProd','$priceProd',
'$modelProd','$marcaProd','$stockProd','$codePProd','".$_FILES['img']['name']."','$adminProd'"))
{
 echo '
 <img src="../assets/img/correctofull.png" class="center-allcontens">
 <br>
 <h3>El producto se añadio a la tienda con éxito</h3>
 <p class="lead text-cente">
 La pagina se redireccionara automaticamente. Si no es asi haga
click en el siguiente boton.<br>
 <a href="../configAdmin.php" class="btn btn-primary btnlg">Volver a administración</a>
 </p>';
 }else{
 echo '

 <img src="../assets/img/incorrectofull.png" class="center-allcontens">
 <br>
...
click en el siguiente boton.<br>
 ...
 La pagina se redireccionara automaticamente. Si no es asi haga
click en el siguiente boton.<br>
 <a href="../configAdmin.php" class="btn btn-primary btn-lg">Volver
a administración</a>
 </p>';
 }
 }else{
